...
## @brief class Fuse_GPS_Odom 用来存储GPS和Encoder的信息
#
#
#
class Fuse_GPS_Odom:
    ...
    ## @property		fuse_encoder
    # Encoder类存储值

    ## @property		fuse_gps_odom_msg
    # 本类融合两种存储值
...
class Encoder:
    ...
    ## @property		left_encoder_val
    # 左里程计值

    ## @property		right_encoder_val
    # 右里程计值

    ## @property		encoder_time
    # 同步里程计时间
...
#
#
# @sa		range_max	超声波最远距离
# @sa		range_min	超声笔最小距离(本来是0.290),range值小于等于最小值,输出无效,固最小值缩小
# @sa		scan_time	超声波扫描时间
# @sa		time_increment	超声波模拟激光模拟数据
# @sa		angle_increment	超声波模拟激光模拟数据
# @sa		angle_min	超声波的探测角(±30°)
# @sa		angle_max	超声波的探测角(±30°)
#
...
## @brief     range_pub range的激光输出函数
#
#    将list格式的range数据转发成ros_laser模式,并通过/range/* topic发出
#
#
# @param		range_msg	range的输入[1,...,3](list)
#
# @b Examples
# @code
#        >>> a = float("inf")
