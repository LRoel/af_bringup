...
class Vo_unit:
    ...
    def update(self, vo_msg):
        self.x = vo_msg.pose.pose.position.x
        self.y = vo_msg.pose.pose.position.y
        self.rate = vo_msg.pose.covariance[10] / vo_msg.pose.covariance[9]
        self.stamp = rospy.Time.now()
        self.odom_x = odom_pose.x
        self.odom_y = odom_pose.y
        self.odom_theta = odom_pose.theta
...
def make_choice():
    ...
    while not rospy.is_shutdown():
        print rospy.Time.now() - vo_pose.stamp <= rospy.Duration(0, 125000000)
        print vo_pose.rate
        if rospy.Time.now() - vo_pose.stamp <= rospy.Duration(0, 125000000) and vo_pose.rate > 0.4:
            out_pose.set_odom(vo_pose.odom_x, vo_pose.odom_y, vo_pose.odom_theta)
            out_pose.x = vo_pose.x
            out_pose.y = vo_pose.y
        else:
            out_pose.x = amcl_output.x
            out_pose.y = amcl_output.y

        out_pose.set_map(out_pose.x, out_pose.y, amcl_output.theta)
        # sendRVIZ()
...
if __name__ == '__main__':
    ...
    rospy.Subscriber('/vo', Odometry, vo_pose.update)
